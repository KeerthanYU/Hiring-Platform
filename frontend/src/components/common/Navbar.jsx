    ...
    <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4">
      <div className="max-w-7xl mx-auto glass rounded-2xl px-6 py-3 flex items-center justify-between border-white/10">
        ...
        <Link to="/" className="flex items-center space-x-2 group">
          ...
          <span className="text-xl font-bold tracking-tight text-white">SkillTest<span className="text-brand-violet">AI</span></span>
        </Link>

        {/* Desktop Nav */}
        <div className="hidden md:flex items-center space-x-8">
          <Link to="/" className={`text-sm font-medium transition-colors ${isActive('/') ? 'text-white' : 'text-slate-400 hover:text-white'}`}>Home</Link>
        ...
        <div className="hidden md:flex items-center space-x-4">
          {!user ? (
            ...
            <div className="flex items-center space-x-4">
              <Link to={user.role === 'recruiter' ? '/recruiter' : '/candidate'} className="text-sm font-medium text-slate-400 hover:text-white transition-colors">
                Dashboard
              </Link>
